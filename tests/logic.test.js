...
const dali = app.destinations.find((item) => item.id === "dali");
assert.strictEqual(
  app.scoreDestination(dali, { ...baseSettings, travelMode: "flight", tripDays: "2" }),
  null,
  "2天1晚不应把明显偏远且行程重的目的地作为常规答案"
);
assert.ok(
  app.scoreDestination(dali, { ...baseSettings, travelMode: "flight", tripDays: "3" }),
  "3天2晚应允许小长假强度目的地进入候选"
);
...
assert.ok(
  [...drawPair.primary.reasons, ...drawPair.surprise.reasons].every((reason) => !reason.includes("本次抽签") && !reason.includes("只保留交通范围")),
  "结果理由不应展示内部抽签口径说明"
);
